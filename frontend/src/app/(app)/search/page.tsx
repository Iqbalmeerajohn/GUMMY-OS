...
/** A flattened, keyboard-navigable result with where it routes to. */
type FlatHit = {
  key: string;
  href: string;
};
...
export default function SearchPage() {
  ...
  const flat: FlatHit[] = [
    ...convHits.map((h) => ({
      ...
      href: `/workspace?c=${h.conversation_id}`,
    })),
    ...msgHits.map((h) => ({
      key: `m:${h.message_id}`,
      href: `/workspace?c=${h.conversation_id}`,
    })),
    ...memHits.map((h) => ({ key: `mem:${h.id}`, href: "/memories" })),
  ];
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <h1 className="font-heading mb-4 text-2xl font-semibold tracking-tight sm:text-3xl">
        Search
      </h1>

      <div className="glass elevation-2 sticky top-20 z-10 mb-6 flex items-center gap-2.5 rounded-2xl px-4 py-3">
          ...
          >
            {convHits.map((h, i) => (
              <ConversationRow
                ...
                onClick={() =>
                  router.push(`/workspace?c=${h.conversation_id}`)
                }
              />
            ))}
          </Group>
    ...
    <li>
      ...
      >
        <span className="block truncate text-sm font-medium">
          <Highlighted text={hit.title ?? "Untitled conversation"} term={term} />
        </span>
  ...
  return (
    <div className="flex flex-col items-center gap-2 px-4 py-16 text-center">
      ...
      <p className="text-muted-foreground text-xs text-balance">
        Find any conversation, message, or memory — type at least{" "}
        {MIN_QUERY} characters.
      </p>
    </div>
  );
}
